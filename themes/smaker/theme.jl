...
;; $Id: theme.jl,v 1.4 1999/09/26 10:11:04 john Exp $
...
(let*
  ...
  (flet
	 ...
	 (or (condition-case nil
       ...
       (rebuild ()
	 (mapc #'(lambda (w)
		   (when (eq (window-get w 'current-frame-style) 'smaker)
	 ...
	 (if smaker:inverted-buttons close-inv-image close-image))
	  ...
	  (if smaker:inverted-buttons min-inv-image min-image))
       ...
       (foreground () smaker:fg-color)
       (bar-dimension () smaker:bar-dimension)
       (-bar-dimension () (- smaker:bar-dimension))
       (title-offset () (- smaker:bar-dimension smaker:border-dimension))
       (border-dimension () smaker:border-dimension)
       (-border-dimension () (- smaker:border-dimension)))
	...
	;; hack custom after-set local functions
	(mapc #'(lambda (sym)
		  (put sym 'custom-after-set (symbol-function 'rebuild)))
	      '(smaker:inverted-buttons smaker:fg-color smaker:bar-dimension
		smaker:border-dimension smaker:image-border))
	(mapc #'(lambda (sym)
		  (put sym 'custom-after-set (symbol-function 'load-images)))
