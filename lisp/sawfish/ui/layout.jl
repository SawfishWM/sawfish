  ...
  (define (document-slot slot)
	...
	(let ((split (tooltip-split doc)))
	  ...
	  (if (memq 'framed (slot-flags slot))
	      (let ((frame (gtk-frame-new doc)))
		(gtk-container-border-width frame box-border)
		(gtk-frame-set-shadow-type frame 'none)
		(gtk-container-add frame (slot-gtk-widget slot))
		(setq frame (add-tooltip frame))
