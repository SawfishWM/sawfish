...
;;; States before maximization
  (define (window-unmaximized-position w)
    (let ((coords (window-position w))  ; (x . y)
          (old-geom (unmaximized-geometry w)))
      (when (window-maximized-horizontally-p w) ;  <--->
        (rplaca coords (nth 0 old-geom))) ;  (x  y width height) ??
  ...
  (define (maximize-discard w #!optional horizontally vertically)
    ...
    (let ((dims (window-dimensions w))
      ...
      (when saved                       ; why not call window-unmaximized-dimensions ??
      ...
      ;; mmc: this should be done first. b/c in this case we don't even need to do the previous step
  ...
  (define (maximize-window w #!optional direction only-1d)
    ...
    (if (window-iconified-p w)
      ...
      (progn
        ...
        (let* ((coords (window-position w))
               (dims (window-dimensions w))
               (fdims (window-frame-dimensions w)) ;(window-dimensions w)); 
          ...
          (when (window-maximizable-p w direction hints)
            ...
            (DB "move-resize-window-to: %s: %d %d x %d %d\n" (window-name w)
...
;;; mmc:
(define (left-upper-quotient position grid)
  (if (< position 0)
      (- (quotient position grid) 1)
    (quotient position grid)))

(define (zero-point-of window)
  (let ((pos (window-position window)))
    (cons
     (* (screen-width) (left-upper-quotient (car pos) (screen-width)))
     (* (screen-height) (left-upper-quotient (cdr pos) (screen-height))))))
...
;; calling the hooks
  (define (unmaximize-window-1 w #!optional direction before)
    (let ((geom (unmaximized-geometry w))
          (coords (window-position w))
          (dims (window-dimensions w))
          (zero-point (zero-point-of w))       ;(cons 0 0)
          )
      (when geom
        (when (memq direction '(() fullscreen horizontal))
          (rplaca coords
                  (+ (nth 0 geom) (car zero-point)) ;mmc: [13 feb 05] (car coords)
                  ;(nth 0 geom)
                  )
          (rplaca dims (nth 2 geom)))
        (when (memq direction '(() fullscreen vertical))
          (rplacd coords
                  (+ (nth 1 geom) (cdr zero-point)) ;(cdr coords)
                  ;(nth 1 geom)
                  )
          (rplacd dims (nth 3 geom)))
        (when before
          (before))
        (DB "unmaximize-window: move-resize-window-to: %s: %d %d x %d %d\n" (window-name w)
            (car coords) (cdr coords)
            (car dims) (cdr dims))
        (move-resize-window-to w (car coords) (cdr coords)
                               (car dims) (cdr dims)))))
...
;;; initialisation
  ...
  (add-hook-s 'after-add-window-hook after-add-window)
...
;; wm-local or X) that mark the window as being maximized
  (mm-add-hook 'before-exit-hook (lambda () (DB "unmaximizing all windows on exit\n") (map-windows unmaximize-window-1))
               ;; fixme:  i could avoid re-making frames due to unmaximizing.
               #f 'unmaximize-window-1)
  ...
  (add-hook-s 'property-notify-hook property-notify)

  (mm-add-hook 'after-initialization-hook
    (lambda ()
      ...
      ;; avoided windows
      (add-hook-s 'add-window-hook check-if-maximizable)) ; fixme: ???
    #f 'check-if-maximizable)

  (mm-add-hook 'sm-window-save-functions
    (lambda (w)
  ...
  (mm-add-hook 'sm-restore-window-hook
    (lambda (w alist)
      (mapc (lambda (cell)
...
;; (add-hook 'after-move-hook maximize-discard-move)
  ...
  (add-hook-s 'after-resize-hook maximize-discard-resize)
