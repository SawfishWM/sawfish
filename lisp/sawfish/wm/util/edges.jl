       ...
       (lambda (w)
         (when (and (window-mapped-p w)
           ...
           (let ((dims (window-frame-dimensions w))
             ...
             (setq x-edges (cons
                            ; open: left
                            ;;  mmc:    start-x start-y end-y
                            (list (car coords) (cdr coords)
                            ...
                            (cons (list (+ (car coords) (car dims))
                                  ...
                                  x-edges)))
             (setq y-edges (cons
                            ;; top:  start-y  start-x  end-x
                            (list (cdr coords) (car coords)
...
;; is the (signed) number of pixels moved since the last call
  (define (edges-within-epsilon list-1 list-2 epsilon delta
                                old-edge #!optional mode) ; (mode 'magnetism)
    (let ((compare
           (cond ((or (null mode) (eq mode 'magnetism))
                  (lambda (e1 e2)
                    (and (< (nth 1 e1) (nth 2 e2)) ; ?? (X Y1 Y2 OPEN-P),   X:  Y1----------------------Y2
                         (> (nth 2 e1) (nth 1 e2)) ;  non-disjointness !!!
                         (<= (- (max (car e1) (car e2)) ;  | i - j | = max - min  
                                (min (car e1) (car e2)))
                             epsilon))))

                 ((eq mode 'attraction)
                  (lambda (e1 e2)
                    (and (< (nth 1 e1) (nth 2 e2)) ;  non-disjointness
                         (> (nth 2 e1) (nth 1 e2))
                         (or
                          (and (nth 3 e1) (not (nth 3 e2)) (<= delta 0) ; 3 ...e1 open (----     ----) e2close     delta movement  <---
                               (<= (- epsilon) (- (car e2) (car e1)) 0) ;  --|e2    |e1--
                               (or (< delta 0) (equal old-edge e2))) ; delta ... moving apart ???
...
;; used to record state across successive calls
  (define (snap-window-position-to-edges window coords deltas state
    ...
    (let* ((dims (window-frame-dimensions window))
           ...
           tem)
      (setq coords (cons (car coords) (cdr coords))) ;  mmc:  ???
