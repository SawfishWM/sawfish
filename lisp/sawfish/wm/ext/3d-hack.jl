...
(define (3d-make-saw)
  (let*
    ...
    (while (< i teeth)
      (let
	  ((out-x (* outer-radius (cos (+ angle step))))
...
(define (3d-make-cube)
  (let
      ((red-gc (x-create-gc 3d-window `((foreground . ,(get-color "red")))))
...
(define (3d-redraw)
  (define (face-visible-p points)
	...
	(let
	    ((p0 (aref 3d-flattened-vertices (aref points 0)))
